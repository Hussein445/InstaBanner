...
const main = async () => {
  const promptMessage = createGradientText("Input your victim's Instagram username without '@'", ['#ff5f6d', '#ffc371']);
  clearAndDisplay(promptMessage);
  const username = await getUserInput('');

  setTimeout(async () => {
    clearAndDisplay('');
    const logMessage = `Reports sending by @MogaTeam to Instagram user @${username}`;
